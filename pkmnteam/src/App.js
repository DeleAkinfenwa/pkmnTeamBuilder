...
function App() {
  ...
  const [pkmn1ID, setPkmn1ID] = useState(1);
  const [pkmn2ID, setPkmn2ID] = useState(3);
  const [pkmn3ID, setPkmn3ID] = useState(4);
  const [pkmn4ID, setPkmn4ID] = useState(6);
  const [pkmn5ID, setPkmn5ID] = useState(7);
  const [pkmn6ID, setPkmn6ID] = useState(9);
    ...
    <div className="App">
      ...
      <footer>
        ...
        <a className='contacts' href='https://github.com/DeleAkinfenwa'>
          <img src={'./project_imgs/GitHub-Mark-Light-64px.png'} alt='github icon'/>
        </a>
      </footer>
    </div>
  );
